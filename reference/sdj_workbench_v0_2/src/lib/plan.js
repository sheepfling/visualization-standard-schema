...
import { findObject, flattenObjects, isShown, validateSemantics } from "./scene.js";
 ...
 * @returns {JsonObject[]}
 */
export function buildAnalysisPlans(scene) {
 ...
 * @returns {JsonObject[]}
 */
export function buildPresentationPlans(scene) {
 ...
 * @returns {JsonObject[]}
 */
export function buildObjectPlans(scene) {
  const capabilities = asObject(scene.backendCapabilities);
 ...
 * @returns {string}
 */
export function fallbackSupport(target, kind) {
  if (target === "cesium") {
 ...
 * @returns {string}
 */
export function artifactForKind(target, kind) {
  if (target === "cesium") {
    if (kind === "tileset") {
 ...
 * @returns {string}
 */
export function artifactPathForKind(target, kind, id) {
 ...
 * @returns {Diagnostic[]}
 */
export function buildTargetDiagnostics(compilePlan, target) {
  /** @type {Diagnostic[]} */
  const diagnostics = [];
 ...
 * @returns {Diagnostic[]}
 */
export function buildObjectDiagnostics(compilePlan, objectId) {
  /** @type {Diagnostic[]} */
  const diagnostics = [];
 ...
 */
export function inspectObject(scene, objectId) {
  ...
  return {
    ...
    object,
    plan: asObject(asArray(compilePlan.objects).map(asObject).find((candidate) => String(candidate.id || "") === objectId)),
    targets: asArray(asObject(asArray(compilePlan.objects).map(asObject).find((candidate) => String(candidate.id || "") === objectId)).targets).map(asObject),
    semanticDiagnostics: [
 ...
 * @returns {string[]}
 */
export function artifactPathsForInspection(target, artifact) {
  if (target === "cesium") {
 ...
 * @returns {string}
 */
export function actionForSupport(target, kind, support, shown) {
  if (!shown) {
 ...
 * @returns {string}
 */
export function encodeArtifactId(id) {
